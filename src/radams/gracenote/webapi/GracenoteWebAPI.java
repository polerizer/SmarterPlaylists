...
public class GracenoteWebAPI
{
    ...
    //Queries with extended argument and returns XML
    public Document searchTrack(String artistName, String albumTitle, String trackTitle, boolean extended, boolean best) throws GracenoteException
    {
        // Sanity check
        if (this._userID.equals("")) { this.register(); }

        String body = this._constructQueryBody(artistName, albumTitle, trackTitle, extended, best);
        String data = this._constructQueryRequest(body);
        String response = this._httpPostRequest(this._apiURL, data);
        return this._checkResponse(response);
    }
    
    //makes a query but doesn't execute it
    public String makeQuery(String artistName, String albumTitle, String trackTitle, boolean extended, boolean best){
        String body = this._constructQueryBody(artistName, albumTitle, trackTitle, extended, best);
        String data = this._constructQueryRequest(body);
        return data;
    }
    ...
    //makes a query but doesn't execute it
    public String makeQuery(String gn_id, boolean extended, boolean best){
        String body = this._constructQueryBody("", "", "", gn_id, "ALBUM_FETCH", extended, best);
    ...
    // This looks up an album directly using it's Gracenote identifier but with extended argument and best argument
    public Document fetchAlbum(String gn_id, boolean extended, boolean best) throws GracenoteException
    {
        // Sanity check
        if (this._userID.equals("")) { this.register(); }
    ...
    //like above, but variable for extended queries and for only picking the best result
    // Constructs the main request body, including some default options for metadata, etc.
    protected String _constructQueryBody(String artist, String album, String track, boolean extended, boolean best) { return this._constructQueryBody(artist, album, track, "", "ALBUM_SEARCH", extended, best); }
    protected String _constructQueryBody(String artist, String album, String track, String gn_id, String command, boolean extended, boolean best)
    {
        String body = "";
